...
|------|-------------|--------|
| `src/app.py` | Streamlit chat UI integrating all three agents into an interactive text-to-SQL interface | `_______` |
| `src/utils/viz.py` | Automatic chart generation module using Altair, selecting chart types based on DataFrame column structure | `_______` |
| `scripts/evaluate.py` | Evaluation harness that runs golden queries, compares results, and writes `artifacts/evaluation_report.json` | `_______` |
| `scripts/generate_golden.py` | Script to generate the golden query benchmark set (`data/golden_queries.json`) | `_______` |
| `data/golden_queries.json` | 50 curated benchmark queries (easy/medium/hard) with ground-truth SQL | `_______` |
| `tests/test_smoke.py` | Smoke tests validating agent contracts, empty-input handling, and config loading | `_______` |
| `README.md` | Project documentation covering setup, architecture, and usage | `_______` |
| `RUN.md` | Quick-start run instructions for reviewers | `_______` |
